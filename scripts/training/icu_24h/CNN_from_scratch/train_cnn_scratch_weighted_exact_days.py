...
with exact_days binning strategy (8 classes, max_days=7) and augmentation.

Base config: configs/icu_24h/weighted_exact_days.yaml (model-agnostic)
Model config: configs/model/cnn_scratch.yaml

To adapt for other models, see: scripts/training/icu_24h/train_with_weighted_exact_days_template.py
"""
...
def main():
    """Main training function for 24h dataset with class weights, exact_days (8 classes), and augmentation."""
    # Load configs - using weighted_exact_days config
    ...
    print("Configuration: exact_days (8 classes), with augmentation")
    print("="*60)
    ...
    if test_loader is not None:
        ...
        # Get number of classes from config (use los_binning if available, else model.num_classes, else default 8)
        try:
            num_classes = get_num_classes_from_config(config)
