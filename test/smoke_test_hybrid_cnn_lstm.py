#!/usr/bin/env python3
"""Smoke test for Hybrid CNN-LSTM training scripts.
...
def test_config(config_name: str, base_config_path: Path, model_config_path: Path):
    ...
    # Check Hybrid CNN-LSTM config
    print(f"\n[2/8] Checking Hybrid CNN-LSTM configuration...")
    ...
    print(f"[OK] CNN config:")
    print(f"    conv1_out: {conv1_out}")
    print(f"    conv2_out: {conv2_out}")
    print(f"    conv3_out: {conv3_out}")
    print(f"[OK] LSTM config:")
    print(f"    hidden_dim: {hidden_dim}")
    print(f"    hidden_dim_layer2: {hidden_dim_layer2}")
    ...
    try:
        ...
        print(f"[OK] HybridCNNLSTM model created")
    ...
    try:
        ...
        # Expected feature dim is 64 (from shared layer output)
...
def main():
    """Run smoke tests for Hybrid CNN-LSTM configurations."""
    print("=" * 80)
    print("Smoke Test: Hybrid CNN-LSTM Training Scripts")
    print("=" * 80)
    
    # Test configurations
    test_configs = [
        {
            "name": "Hybrid CNN-LSTM 24h Weighted (SQRT) - From Scratch",
            "base_config": Path("configs/icu_24h/24h_weighted/sqrt_weights.yaml"),
            "model_config": Path("configs/model/hybrid_cnn_lstm/hybrid_cnn_lstm.yaml"),
        },
    ]
    ...
    if all_passed:
        ...
        print("Hybrid CNN-LSTM training scripts are ready to use.")
    else:
