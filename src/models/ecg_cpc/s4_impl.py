...
try:
    ...
    # Fallback if pytorch_lightning not available
...
try:  # Try pykeops
    ...
    if not has_cauchy_extension:
        log.warn(
            "Neither pykeops nor cauchy_mult extension found. Using slower CPU fallback for cauchy multiplication."
        )
...
def cauchy_conj(v, z, w):
    """ Pykeops version (if available), otherwise CPU fallback """
    if has_pykeops:
